...
def needs_sqlite(f, self, *a, **kw):
    ...
    return [] if sqlite3 is None or not self.enabled else f(self, *a, **kw)
...
class HistoryAccessor(HistoryAccessorBase):
    ...
    def _run_sql(self, sql, params, raw=True, output=False):
        ...
        cur = self.db.execute(
            f"SELECT session, line, {toget} FROM {sqlfrom} {sql}", params
        )
    ...
    def get_tail(self, n=10, raw=True, output=False, include_latest=False):
        ...
        return reversed(list(cur)[1:]) if not include_latest else reversed(list(cur))
