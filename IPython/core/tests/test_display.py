...
def test_image_size():
    ...
    nt.assert_equal(
        f'<img src="{thisurl}" width="200" height="200"/>', img._repr_html_()
    )
    ...
    nt.assert_equal(f'<img src="{thisurl}" width="200"/>', img._repr_html_())
    img = display.Image(url=thisurl)
    nt.assert_equal(f'<img src="{thisurl}"/>', img._repr_html_())
    img = display.Image(url=thisurl, unconfined=True)
    nt.assert_equal(
        f'<img src="{thisurl}" class="unconfined"/>', img._repr_html_()
    )
